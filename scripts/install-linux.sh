...
log "step 4/9: resolving linux asset URL"
asset_url="$(printf '%s' "${json}" | tr '\n' ' ' | grep -Eo '"browser_download_url":"[^"]+"' | sed -E 's/^"browser_download_url":"(.*)"$/\1/' | grep -E 'cognautic-forge-linux\.tar\.gz$' | head -n1)"
if [[ -z "${asset_url}" ]]; then
  echo "Could not find cognautic-forge-linux.tar.gz in latest release"
  exit 1
fi
log "asset=${asset_url}"
